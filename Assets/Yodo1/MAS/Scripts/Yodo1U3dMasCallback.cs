...
namespace Yodo1.MAS
{
    public class Yodo1U3dMasCallback : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            _allAudioSources = (FindObjectsOfType(typeof(AudioSource)) as AudioSource[]).ToList();
#endif
        }
...
#if UNITY_EDITOR || UNITY_ANDROID
            Time.timeScale = 0;
            PauseAllAudio(_allAudioSources);
#endif
        }
...
#if UNITY_EDITOR || UNITY_ANDROID
            Time.timeScale = 1;
            UnPauseAllAudio(_allAudioSources);
#endif
        }

        private void PauseAllAudio(List<AudioSource> allAudioSources)
        {
            for (int i = 0; i < allAudioSources.Count; i++)
            {
                ...
                {
                    allAudioSources.RemoveAt(i);
                }
            }
        }

        private void UnPauseAllAudio(List<AudioSource> allAudioSources)
        {
            for (int i = 0; i < allAudioSources.Count; i++)
            {
